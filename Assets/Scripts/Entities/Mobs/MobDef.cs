...
public class MobDef : EntityDef
{
    ...
    [Tooltip("Combat behavior settings. Set canAttack=true to make mob hostile.")]
    public CombatConfig combatConfig = CombatConfig.Passive;
...
#if UNITY_EDITOR
    [ContextMenu("Make Passive")]
    private void MakePassive()
    {
        combatConfig = CombatConfig.Passive;
        UnityEditor.EditorUtility.SetDirty(this);
    }
    
    [ContextMenu("Make Aggressive (Attacks Adventurers)")]
    private void MakeAggressive()
    {
        combatConfig = CombatConfig.AggressiveToAdventurers;
        UnityEditor.EditorUtility.SetDirty(this);
    }
#endif
}